...
class NetLSD(Estimator):
    ...
    def _updown_linear_approx(
        self, eigenvalues_lower, eigenvalues_upper, number_of_nodes
    ):
        """
        ...
        eigenvalues[nal - 1 : -nau + 1] = np.linspace(
            eigenvalues_lower[-1], eigenvalues_upper[0], number_of_nodes - nal - nau + 2
        )
        return eigenvalues
...
if __name__ == "__main__":
    ...
    # ---- initialize NetLSD (500-dim) ----
    model = NetLSD(
        scale_steps=500,   # embedding dimension
        approximations=50, # small graph, keep it small
        seed=42,
    )
